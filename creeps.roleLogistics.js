module.exports = {
	...
	let logiPriority = [{"task": "repair", "coeff": 0.25, "exclude1": STRUCTURE_WALL, "exclude2": STRUCTURE_RAMPARTS},
			...
			case "unload":
				...
				for (i = 0, i<6 && repStruc == undefined && bldStruc == undefined, i++){
					switch (logiPriority[i]["task"])
						case "repair":
							let repStruc = creep.pos.findClosestByPath(FIND_STRUCTURES, {
								...
								s.structureType != logiPriority[i]["exclude1"] &&
								s.structureType != logiPriority[i]["exclude2"]
							});
						break;
						case "build":
					...
					if (bldStruc != undefined) {
						if (creep.build(bldStruc) == ERR_NOT_IN_RANGE) {creep.moveTo(bldStruc);}
					}
				}
			break;
}